...
<body>
    ...
    <main>
        ...
        @php
            $user = Auth::user();
            $checkCartUser = \DB::table('cart')
                ->where('id_user', $user->id)
                ->get();
        @endphp
        <section id="product-cart">
            @if ($checkCartUser->isEmpty())
                <div class="empty-cart container">
                    <div class="empty-wrap">
                        <div class="empty-img">
                            <img src="{{ url('fkhco/assets/png/payment/empty.webp') }}" alt="">
                        </div>
                        <div class="empty-desc">
                            <h2>Oops! Your cart is empty!</h2>
                            <p>Looks like you haven't added anything to your cart yet.</p>
                            <a href="{{ url('/products') }}" class="view-btn shop-btn">Shop Now</a>
                        </div>
                    </div>
                </div>
            @else
                                    ...
                                    <div class="product-wishlist">
                                        <div class="detail-cart">
                                            ...
                                            <div class="product-details">
                                                ...
                                                <p class="size-chart">Size: <span>{{ strtoupper($cart->ukuran) }}</span>
                                                </p>
                                                <p class="price cart-price">
                                                    {{ 'Rp. ' . number_format($cart->harga_jual, 0, ',', '.') }}</p>
                                            </div>
                                        </div>
                                        <div class="btn-feat">
                                            <form action="{{ Route('cart.destroy', $cart->id_cart) }}" method="POST"
                                                ...
                                                <button class="trash-btn">
                                                    <img src="{{ url('fkhco/assets/svg/trash.svg') }}"
                                                        alt="trash-icon" />
                                                </button>
                                            </form>
                                            ...
                                            <form action="{{ Route('cart.update', $cart->id_cart) }}" method="POST">
                                                ...
                                                <div class="quantity qty">
                                                    <button id="decrease" name="action" value="decrease"
                                                        class="counter" type="submit">-</button>
                                                    <input type="number" class="qty" name="qty" id="qty"
                                                        value="{{ $cart->qty }}" max="100" required />
                                                    <button id="increase" name="action" value="increase"
                                                        class="counter" type="submit">+</button>
                                                </div>
                                            </form>
                                        </div>
